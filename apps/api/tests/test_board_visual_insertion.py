...
from app.services.board_visual_insertion import apply_board_insertion_plan, build_board_insertion_plan
...
def test_board_asset_store_rejects_decompressed_pixel_budget_overflow(
    ...
    with pytest.raises(BoardAssetError, match="dimensions"):
...
def test_visuals_from_different_chapters_may_follow_document_teaching_order(tmp_path) -> None:
    store = _store(tmp_path)
    visuals = [
