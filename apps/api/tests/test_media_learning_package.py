...
from app.services.media_access import YouTubeAccessConfig, is_youtube_url
...
from app.services.media_visual_extraction import (
    FrameSample,
    bounded_frame_samples,
    detect_reset_intervals,
)
...
def test_youtube_url_detection_is_host_bounded() -> None:
    assert is_youtube_url("https://youtu.be/video") is True
    assert is_youtube_url("https://www.youtube.com/watch?v=video") is True
    assert is_youtube_url("https://example.com/youtube.com/watch?v=video") is False
...
def test_ytdlp_youtube_access_uses_js_runtime_and_only_explicit_browser_authorization(
    monkeypatch,
) -> None:
    import yt_dlp
    ...
    monkeypatch.setattr(
        socket,
        "getaddrinfo",
        lambda *_args, **_kwargs: [(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("142.250.72.206", 0))],
    )
    monkeypatch.setattr(yt_dlp, "YoutubeDL", FakeDownloader)
    monkeypatch.setattr(
        "app.services.media_adapters.youtube_access_config",
        lambda: YouTubeAccessConfig(
            js_runtime="node",
            js_runtime_path="/opt/node/bin/node",
            ejs_available=True,
            browser_authorization_enabled=True,
            browser_name="chrome",
    ...
    adapter.resolve("https://www.youtube.com/watch?v=video")
    adapter.resolve(
        "https://www.youtube.com/watch?v=video",
        use_browser_authorization=True,
    )

    assert captured[0]["js_runtimes"] == {"node": {"path": "/opt/node/bin/node"}}
    assert "cookiesfrombrowser" not in captured[0]
    assert captured[1]["cookiesfrombrowser"] == ("chrome", None, None, None)


def test_ytdlp_bot_challenge_has_auditable_authorization_error(monkeypatch) -> None:
    import yt_dlp
    ...
    monkeypatch.setattr(
        ...
        lambda: YouTubeAccessConfig(
            ...
            js_runtime_path="/opt/node/bin/node",
            ejs_available=True,
            browser_authorization_enabled=False,
            browser_name="chrome",
            browser_profile=None,
            cookie_file=None,
        ),
    ...
    with pytest.raises(MediaAdapterError) as captured:
        YtDlpMediaAdapter().resolve("https://www.youtube.com/watch?v=video")

    assert captured.value.code == "youtube_authorization_required"
...
def test_frame_analysis_budget_preserves_timeline_endpoints() -> None:
    samples = [
        FrameSample(index * 1_000, np.zeros((2, 2), dtype=np.uint8), b"jpeg")
        for index in range(100)
    ]

    bounded = bounded_frame_samples(samples, limit=12)

    assert len(bounded) == 12
    assert bounded[0].timestamp_ms == 0
    assert bounded[-1].timestamp_ms == 99_000
...
def test_local_transcription_default_does_not_create_an_openai_pseudo_model(
    monkeypatch,
) -> None:
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.setenv("OPENCLASS_DEFAULT_TRANSCRIPTION_PROVIDER", "openclass_local")
    monkeypatch.setattr(
        ai_model_catalog,
    ...
    assert catalog.defaults["transcription"].provider == "openclass_local"
    assert not any(
        option.provider == "openai" and option.model == "local-whisper"
        for option in catalog.transcription
    )
...
def test_pipeline_keeps_subtitle_package_usable_when_visuals_are_empty(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("subtitle path must not transcribe")
        ),
    )
    segments = [_segment(index) for index in range(4)]
