...
def test_codex_binary_path_prefers_path_discovery(monkeypatch) -> None:
    ...
    monkeypatch.setattr(codex_app_server.shutil, "which", lambda _name: "/opt/bin/codex")
...
def test_codex_binary_path_falls_back_to_chatgpt_bundle(monkeypatch) -> None:
    ...
    monkeypatch.setattr(os, "access", lambda path, mode: path == bundled_binary and mode == os.X_OK)
