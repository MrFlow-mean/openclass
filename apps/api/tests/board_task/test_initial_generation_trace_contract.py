...
TEST_USER_ID = "user_initial_generation_trace_contract"
TRACE_KEYS = {
    "workflow_trace",
    "workflow_steps",
    "workflow_node_id",
...
CONTRACT_XFAIL_REASON = (
    "Wave 5 preparation contract only: Coordinator owns final chatbot/workflow_trace wiring."
)
...
def _patch_ready_generation_common(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        _ready_requirement_update,
    )


@pytest.mark.xfail(reason=CONTRACT_XFAIL_REASON, strict=True)
def test_initial_ready_generation_trace_contract_records_freeze_generate_commit_consume(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    workspace, lesson_id = _workspace_with_blank_lesson()
    store = _store_with_workspace(tmp_path, workspace, name="initial_generation_success_contract")
    ...
    monkeypatch.setattr(
        ...
        _fake_generate_from_requirements,
    )
...
@pytest.mark.xfail(reason=CONTRACT_XFAIL_REASON, strict=True)
def test_initial_generation_failure_trace_contract_keeps_frozen_run_retryable(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    workspace, lesson_id = _workspace_with_blank_lesson()
    store = _store_with_workspace(tmp_path, workspace, name="initial_generation_failure_contract")
    ...
    monkeypatch.setattr(
        chatbot_module,
        "generate_from_requirements",
        _failed_generate_from_requirements,
    )
