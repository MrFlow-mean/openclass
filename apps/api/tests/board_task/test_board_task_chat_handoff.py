...
def _workspace_inputs() -> dict[str, Any]:
    ...
    return {
        ...
        "package": package,
        "lesson": lesson,
        "request": ChatRequest(message="开始规则互动", selection={"kind": "board", "excerpt": focus.excerpt}),
...
def _deps(calls: dict[str, Any], *, start_response: object | None) -> BoardTaskChatHandoffDependencies:
...
def _call_handler(inputs, calls, *, start_response: object | None = None):
    ...
    return handle_board_task_chat_handoff(
        ...
        deps=_deps(calls, start_response=response),
    )
...
def test_chat_handoff_builds_task_requirements_and_delegates_to_interaction_start() -> None:
    ...
    with bind_workflow_trace_collector() as collector:
        response = _call_handler(inputs, calls, start_response=start_response)
