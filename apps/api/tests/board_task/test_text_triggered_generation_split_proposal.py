...
TEST_USER_ID = "user_text_triggered_generation_split"
...
def test_classifier_is_pure_and_owns_only_blank_board_text_generation_after_resource_precedence() -> None:
    ...
    lesson_snapshot = lesson.board_document.model_dump(mode="json")
    ...
    assert lesson.board_document.model_dump(mode="json") == lesson_snapshot
...
def test_handler_success_freezes_before_generate_commits_consumes_saves_then_assembles(
    ...
    with bind_workflow_trace_collector() as collector:
        response = handle_text_triggered_generation(
