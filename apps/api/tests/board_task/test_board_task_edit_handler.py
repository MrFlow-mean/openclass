...
from app.services.chat.paths import board_task_edit as board_task_edit_module
from app.services.chat.paths.board_task_edit import BoardTaskEditHandlerDependencies, handle_board_task_edit
...
def _workspace_inputs():
    ...
    return {
        ...
        "package": package,
        "lesson": lesson,
        "request": ChatRequest(message="请改写这段", selection={"kind": "board", "excerpt": "原始内容需要被改写。"}),
        "requirements": requirements,
        "learning_clarification": LearningClarificationStatus(**clarification),
...
class _BoardTaskHistoryStub:
    def __init__(self, *, fail_consume: bool = False) -> None:
        ...
        self.failed_stamp = BoardTaskHistoryStamp(run_id="run-edit", version_id="ver-failed", phase="execution_failed")
...
def _deps(calls: dict[str, Any], *, edit_outcome: BoardDocumentEditOutcome) -> BoardTaskEditHandlerDependencies:
    ...
    def build_response(**kwargs):
        ...
        return SimpleNamespace(
            ...
            board_task_phase=kwargs.get("board_task_stamp").phase if kwargs.get("board_task_stamp") else None,
    ...
    return BoardTaskEditHandlerDependencies(
...
def _call_handler(inputs, calls, history, outcome):
    return handle_board_task_edit(
        ...
        deps=_deps(calls, edit_outcome=outcome),
    )


def test_handler_success_persists_ready_commit_consume_and_response(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    original_commit_operations = board_task_edit_module.commit_operations
    ...
    monkeypatch.setattr(board_task_edit_module, "commit_operations", capture_commit_metadata)

    with bind_workflow_trace_collector() as collector:
        response = _call_handler(inputs, calls, history, _success_outcome(inputs["lesson"]))
...
def test_handler_consume_failure_does_not_clear_save_or_assemble_response() -> None:
    inputs = _workspace_inputs()
    calls: dict[str, Any] = {}
