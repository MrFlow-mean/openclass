...
def test_existing_board_edit_success_current_behavior_persists_commit_and_consumes_task(
    ...
    store = _store_with_workspace(tmp_path, workspace, name="board_task_edit_success_current")
    monkeypatch.setattr(workspace_state, "STORE", store)
    monkeypatch.setattr(
        openai_course_ai,
        "generate_board_task_requirement_sheet",
...
def test_existing_board_edit_target_miss_current_behavior_keeps_task_active(
    ...
    store = _store_with_workspace(tmp_path, workspace, name="board_task_edit_target_miss_current")
    monkeypatch.setattr(workspace_state, "STORE", store)
...
def test_existing_board_edit_execution_failed_current_behavior_records_history_without_commit(
    ...
    original_document = lesson.board_document.model_dump(mode="json")
    initial_commit_count = len(lesson.history_graph.commits)
    store = _store_with_workspace(tmp_path, workspace, name="board_task_edit_failed_current")
    monkeypatch.setattr(workspace_state, "STORE", store)
    monkeypatch.setattr(
        openai_course_ai,
        "generate_board_task_requirement_sheet",
...
def test_existing_board_repeated_missing_edit_current_behavior_converts_to_write_confirmation(
    ...
    lesson = workspace.packages[0].lessons[-1]
    original_document = lesson.board_document.model_dump(mode="json")
    store = _store_with_workspace(tmp_path, workspace, name="board_task_edit_repeated_miss_current")
    monkeypatch.setattr(workspace_state, "STORE", store)
    monkeypatch.setattr(openai_course_ai, "generate_board_task_requirement_sheet", lambda **kwargs: None)
    monkeypatch.setattr(
        openai_course_ai,
