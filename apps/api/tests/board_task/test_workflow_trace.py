...
def test_existing_board_missing_task_fields_current_terminal_behavior_golden(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    workspace, lesson_id = _workspace_with_lesson(existing_board=True)
    store = _store_with_workspace(tmp_path, workspace, name="board_task_missing_fields_current")
    monkeypatch.setattr(workspace_state, "STORE", store)
...
def test_existing_board_clarify_location_current_terminal_behavior_golden(
    ...
    workspace, lesson_id = _workspace_with_lesson(existing_board=True)
    lesson = workspace.packages[0].lessons[-1]
    store = _store_with_workspace(tmp_path, workspace, name="board_task_clarify_location_current")
    monkeypatch.setattr(workspace_state, "STORE", store)
    _patch_board_task_terminal_guards(monkeypatch, chatbot_message="AI生成：你想让我处理哪一段？")
    first_focus = _generic_focus(lesson.id, lesson.board_document.id, label="第一处", excerpt="第一处内容。", segment_id="seg_one")
    second_focus = _generic_focus(lesson.id, lesson.board_document.id, label="第二处", excerpt="第二处内容。", segment_id="seg_two")
...
def test_existing_board_await_write_confirmation_current_terminal_behavior_golden(
    ...
    workspace, lesson_id = _workspace_with_lesson(existing_board=True)
    store = _store_with_workspace(tmp_path, workspace, name="board_task_await_write_current")
    monkeypatch.setattr(workspace_state, "STORE", store)
...
def test_existing_board_write_confirmation_decline_current_terminal_behavior_golden(
    ...
    workspace, lesson_id = _workspace_with_lesson(existing_board=True)
    store = _store_with_workspace(tmp_path, workspace, name="board_task_decline_current")
    monkeypatch.setattr(workspace_state, "STORE", store)
    _patch_board_task_terminal_guards(monkeypatch, chatbot_message="AI生成：当前板书没有这部分内容。")
    resolution = FocusResolution(
        focus=None,
