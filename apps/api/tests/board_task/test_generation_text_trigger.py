...
def _draft_state(*, actionable: bool = True, work_mode: str | None = None):
    requirements = build_requirements("通用学习主题")
    requirements.work_mode = work_mode
...
def test_document_artifact_text_classifies_without_mutating_state() -> None:
    ...
    assert classified.trigger == "document_artifact_request"
...
def test_classifier_excludes_api_resource_existing_board_and_knowledge_board_paths() -> None:
    lesson = _blank_lesson()
    requirements, clarification = _draft_state()
    ...
    knowledge_requirements, knowledge_clarification = _draft_state(work_mode="knowledge_board")
    assert (
        classify_text_triggered_generation_request(
            lesson=lesson,
            request=ChatRequest(message="请生成一份学习材料"),
            requirements=knowledge_requirements,
            learning_clarification=knowledge_clarification,
            resource_resolution=base_resolution,
        )
        is None
    )
