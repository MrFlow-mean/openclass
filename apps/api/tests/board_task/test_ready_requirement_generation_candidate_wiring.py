...
TEST_USER_ID = "user_ready_requirement_generation_candidate_wiring"
TRACE_KEYS = {
    "workflow_trace",
    "workflow_steps",
    "workflow_node_id",
...
def test_candidate_wiring_ready_success_matches_slot_b_trace_contract(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    workspace, lesson_id = _workspace_with_blank_lesson()
    store = _store_with_workspace(tmp_path, workspace, name="candidate_ready_generation_success")
...
def test_candidate_wiring_ready_failure_matches_slot_b_trace_contract(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    workspace, lesson_id = _workspace_with_blank_lesson()
    store = _store_with_workspace(tmp_path, workspace, name="candidate_ready_generation_failure")
...
def test_candidate_wiring_does_not_handle_explicit_board_generation_start(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    workspace, lesson_id = _workspace_with_blank_lesson()
    store = _store_with_workspace(tmp_path, workspace, name="candidate_explicit_start_scope")
    monkeypatch.setattr(workspace_state, "STORE", store)
    _patch_ready_generation_common(monkeypatch)
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: pytest.fail("candidate handler must not handle explicit start"),
    )
...
def test_candidate_wiring_does_not_handle_confirmed_resource_generation(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    workspace, lesson_id = _workspace_with_resource(tmp_path)
    store = _store_with_workspace(tmp_path, workspace, name="candidate_confirmed_resource_scope")
    monkeypatch.setattr(workspace_state, "STORE", store)
    _patch_ready_generation_common(monkeypatch)
    monkeypatch.setattr(
        chatbot_module,
    ...
    monkeypatch.setattr(
        chatbot_module,
        "handle_ready_requirement_generation",
        lambda **kwargs: pytest.fail("candidate handler must not handle confirmed-resource generation"),
    )
    monkeypatch.setattr(
        chatbot_module,
        "generate_from_requirements",
...
def test_candidate_wiring_does_not_handle_knowledge_board_special_entry(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    workspace, lesson_id = _workspace_with_blank_lesson()
    store = _store_with_workspace(tmp_path, workspace, name="candidate_knowledge_scope")
    monkeypatch.setattr(workspace_state, "STORE", store)
    _patch_ready_generation_common(monkeypatch)
    monkeypatch.setattr(
        openai_course_ai,
    ...
    monkeypatch.setattr(
        chatbot_module,
        "handle_ready_requirement_generation",
        lambda **kwargs: pytest.fail("candidate handler must not handle knowledge_board generation"),
    )
