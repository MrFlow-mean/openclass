...
def _workspace_context():
    ...
    lesson = create_empty_lesson("等待写入确认处理器测试")
...
def _route_decision() -> BoardTaskRouteDecision:
    return BoardTaskRouteDecision(
        ...
        reason="板书没有对应内容，需要先确认是否扩写。",
        write_proposal="补充缺失主题的简短说明。",
        target_scope="append",
    )
...
def _make_deps(
    *,
    ...
    def _message(**kwargs):
        ...
        message_contexts.append(kwargs["context"])
        return (
            f"AI生成：板书里还没有“{board_task.question_or_topic}”，要先扩写吗？",
...
def _run_handler(
    *,
    ...
    return handle_board_task_await_write_confirmation_terminal(
        ...
        request=ChatRequest(message="请讲一下这个缺失主题"),
...
def test_terminal_sets_absent_write_confirmation_and_records_trace_after_save() -> None:
    ...
    assert captured["message_contexts"] == [
        "板书里没有对应内容。请询问用户是否要先扩写板书，再继续学习。"
    ]
...
def test_rejects_non_await_write_confirmation_route() -> None:
    workspace, package, lesson = _workspace_context()
    requirement_history, board_task_history = _histories(lesson)
    deps, _ = _make_deps(lesson=lesson)
