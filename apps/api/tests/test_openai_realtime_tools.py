...
from app.models import RealtimeConnectRequest, RealtimeToolCallRequest, SelectionRef
...
def test_chatbot_tool_reuses_existing_workflow(monkeypatch, isolated_store) -> None:
    ...
    def _fake_chat(lesson_id, request, *, user_id):
        captured.update(lesson_id=lesson_id, message=request.message, user_id=user_id)
