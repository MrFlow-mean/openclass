...
def resolve_source_grounded_board_plan(
    *,
    ...
    if is_whole_source:
        return _resolve_open_notebook_source_plan(
            owner_user_id=owner_user_id,
            package_id=package.id,
            lesson=lesson,
            source=source,
...
def _resolve_open_notebook_source_plan(
    *,
    owner_user_id: str,
    package_id: str,
    lesson: Lesson,
    ...
    if not source.open_notebook_source_id:
        raise SourceGroundedBoardError("这份资料还没有完成 OpenNotebook 处理，请稍后重试。")
    ...
    if not evidence:
        raise SourceGroundedBoardError("OpenNotebook 没有在这份资料中找到与本轮问题相关的正文。")
