...
class TextTriggeredGenerationResponseBuilder(Protocol):
    def __call__(
        self,
        *,
        workspace: WorkspaceState,
...
@dataclass(frozen=True)
class TextTriggeredGenerationDependencies:
    ...
    build_response: TextTriggeredGenerationResponseBuilder
...
def handle_text_triggered_generation_request(
    *,
    generation_request: TextTriggeredGenerationRequest,
    workspace: WorkspaceState,
    package: CoursePackage,
    lesson: Lesson,
    user_id: str,
    ...
    _assert_text_triggered_generation_scope(
        lesson=lesson,
        request=request,
    ...
    if frozen_requirement is not None:
        record_workflow_step(
            ...
            decision="frozen",
            reason=generation_request.trigger,
    ...
    deps.commit_operations(
        ...
        message="Generated board document from a text-triggered generation request",
        ...
        metadata={
            ...
            "generation_request_trigger": generation_request.trigger,
            "generation_request_trigger_reason": generation_request.reason,
...
def _assert_text_triggered_generation_scope(
    *,
    lesson: Lesson,
    request: ChatRequest,
    resource_resolution: ResourceResolution,
    ...
    if request.board_generation_action == "start":
        raise ValueError("text-triggered generation handler does not own API board_generation_action=start")
    if request.resource_reference_action is not None:
        raise ValueError("text-triggered generation handler does not own resource confirmation turns")
    if resource_resolution.reference_prompt is not None:
        raise ValueError("resource reference prompt must be handled before text-triggered generation")
    if not is_document_empty(lesson.board_document):
        raise ValueError("text-triggered generation handler only owns blank-board generation")
