...
class IntentSignals:
    ...
    wants_strong_explanation: bool
    wants_append: bool
    wants_expand: bool
    wants_simplify: bool
    wants_rewrite: bool
    has_target_location_hint: bool
    has_resource_reference_hint: bool
    has_explicit_resource_reference: bool
    wants_learning_start: bool
    is_followup_execution: bool
    wants_document_artifact_generation: bool

    @property
    def has_write_intent(self) -> bool:
        return self.wants_append or self.wants_expand or self.wants_rewrite or self.wants_simplify
...
def extract_intent_signals(text: str | None) -> IntentSignals:
    compact = compact_text(text, limit=280)
    return IntentSignals(
        compact=compact,
        wants_explanation=bool(compact and EXPLAIN_REQUEST_PATTERN.search(compact)),
        wants_strong_explanation=bool(compact and STRONG_EXPLAIN_REQUEST_PATTERN.search(compact)),
        wants_append=bool(compact and APPEND_REQUEST_PATTERN.search(compact)),
        wants_expand=bool(compact and EXPAND_REQUEST_PATTERN.search(compact)),
        wants_simplify=bool(compact and SIMPLIFY_REQUEST_PATTERN.search(compact)),
        wants_rewrite=bool(compact and REWRITE_REQUEST_PATTERN.search(compact)),
        has_target_location_hint=bool(compact and TARGET_LOCATION_HINT_PATTERN.search(compact)),
        has_resource_reference_hint=bool(compact and RESOURCE_REFERENCE_HINT_PATTERN.search(compact)),
        has_explicit_resource_reference=bool(compact and EXPLICIT_RESOURCE_REFERENCE_PATTERN.search(compact)),
        wants_learning_start=bool(compact and LEARNING_START_REQUEST_PATTERN.search(compact)),
        is_followup_execution=bool(compact and FOLLOWUP_EXECUTION_PATTERN.search(compact_text(text, limit=80))),
        wants_document_artifact_generation=bool(compact and DOCUMENT_ARTIFACT_REQUEST_PATTERN.search(compact)),
    )
