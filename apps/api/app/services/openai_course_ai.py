    ...
    def generate_blank_board_requirement_refinement(
        ...
        *,
        ...
        system_prompt = (
            ...
            "current_level 只能在有可核验来源时填写：用户本轮明确自述时标记 current_level_source=user_statement 并"
            "在 current_level_evidence 原样摘录该自述；用户选择上一轮水平卡片时标记 level_profile_choice；"
            "沿用已验证事实时标记 existing_requirement。不得把学习入口、推荐路线或模型推断写成 current_level。"
            "当输出选择卡片时，每个 option 必须给 selection_key；只有描述用户已有能力、经验或卡点的卡片才使用"
            "option_kind=level_profile，并填写 level_profile。内容路线必须使用 option_kind=learning_entry，"
            "且选择内容路线不能证明用户水平。"
