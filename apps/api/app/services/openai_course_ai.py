    ...
    def generate_basic_chat_reply(
        ...
        *,
        ...
        system_prompt = (
            ...
            "聊天框不能生成或复述可直接写入右侧文档的完整板书、章节、讲义、代码实现或代码围栏。"
            "用户要求生成、补写、改写或扩展文档时，不要在聊天框给出正文；只简短承接该请求。"
        )
        user_prompt = _json(
            {
                ...
                "response_contract": {
                    "chatbot_message": "直接回复用户当前问题；可以简短解释或举例，但不得输出完整可写入文档的正文、章节、代码实现或代码围栏。",
                },
            }
        )
        result = self._parse(
    ...
    def generate_learning_intake_reply(
        ...
        *,
        ...
        system_prompt = (
            ...
            "如果 requirement_context.granularity=source_chapter，表示当前指定章节已经精确解析并已解决“学什么内容”；"
            "此时不得再要求用户从章节内选择子主题，只可提示确认候选资料或追问真正独立的缺项。\n"
    ...
    def generate_blank_board_requirement_refinement(
        ...
        *,
        ...
        system_prompt = (
            ...
            "knowledge_board 用于学习新知识：具体到单一知识点、概念、方法、步骤或问题时可 ready_for_board=true；"
            "若权威 initial_work_mode_decision 的 granularity=source_chapter，表示用户指定的已验证资料章节已经精确解析，"
            "章节本身就是 learning_goal 和 boundary，必须保持 source_chapter，不得追问章节内子主题，并可 ready_for_board=true。"
            "这只解决学习内容，不等于用户已确认候选资料。宽泛主题继续用一个最有价值的问题收敛起点。"
            "practice_artifact 用于练习、测验、角色、问答或案例等可操练产物，"
    ...
    def generate_chatbot_reply(
        ...
        *,
        ...
        system_prompt = (
            ...
            "4a. 当 interaction_context 中有已批准的 board_explanation_directive，或本轮明确给出了可讲解的目标摘录时，"
            "请像老师正在当面讲这一小段内容，而不是像课件、教案或课程流程播报。先把眼前的结论说清，"
            "再用目标摘录中的因果、条件、步骤或关系把“为什么/怎么理解”讲出来；需要例子或类比时，"
            "只根据当前内容自然选择，不要套用固定例子。话语应能接住学习者当前的问题和最近对话。\n"
            "4b. 讲解结束时按当前内容自然收束：不要机械布置“用自己的话复述”、统一检查是否听懂，"
            "也不要例行预告下一节或把讲解变成课堂流程说明。只有确实能帮助理解时，才提出一个具体、"
            "可回答的问题；不要为了互动而提问。\n"
            "4c. 当目标摘录含有图表、示意图、关系图、流程图、树图、表格或代码块形式的视觉内容时，"
            "聊天框只负责用自然语言讲解：说明图中有哪些对象、它们之间的关系或变化顺序，以及图支持的结论。"
            "不得把原图、原表、字符画、代码围栏或按空间排布的内容原样搬进聊天框；即使图本身不清晰，"
            "也应依据可读内容做文字说明，而不是复刻视觉排版。\n"
        ...
        user_prompt = _json(
            {
                ...
                "user_message": user_message,
                "response_contract": {
                    "chatbot_message": "面向学习者的自然语言短回复；不要输出整篇可写入文档区的正文。讲解板书视觉内容时，只描述和解释，不复制原图、原表、字符画或代码围栏。",
                },
            }
        )
        result = self._parse(
    ...
    def generate_board_explanation_directive(
        ...
        *,
        ...
        system_prompt = (
            ...
            "2a. 目标摘录含有图表、示意图、关系图、流程图、树图或表格时，teaching_instruction 必须说明"
            "Chatbot 应如何解释对象、关系和结论；不得要求 Chatbot 复刻图形、表格、字符画或代码围栏。\n"
    ...
    def generate_board_task_requirement_refinement(
        ...
        *,
        ...
        system_prompt = (
            ...
            "12. 聊天框不能生成或复述可直接写入右侧文档的板书正文：不得输出完整章节、完整代码实现、"
            "代码围栏、讲义式大纲或可直接粘贴的文档片段。用户明确要求写入或改写时，必须选择 "
            "board_task_refining 并只给出简短的任务承接、确认或一个必要问题；正文只能由后端文档执行链写入右侧。\n"
            "13. 不写任何学科、教材、考试或样例专属规则。"
        )
        user_prompt = _json(
            {
                "lesson_title": lesson_title,
                ...
                "response_contract": {
                    ...
                    "chatbot_message": (
                        ...
                        "不要输出内部字段名、JSON、表单格式或可写入右侧文档的完整正文。"
                    ),
                    "board_task_sheet": {
                        "board_workflow": "固定写 act_on_existing_board，表示对已有板书内容做动作。",
                        "location_kind": "target_range、insertion_anchor 或 unspecified。",
    ...
    def generate_initial_learning_work_mode(
        ...
        *,
        ...
        system_prompt = (
            ...
            "6. source_chapter 仅能由后续已验证资料章节解析产生；本阶段不得仅因文字提到章节就输出 source_chapter。\n"
            "7. guided_discovery_reply 必须像 Chatbot 在左侧聊天框自然说话：基于 lesson_title、resource_summary、"
    ...
    def generate_board_document_edit(
        ...
        *,
        ...
        system_prompt = (
            ...
            "把 title 作为唯一总标题；content_text 不得再次输出与 title 相同的 H1。正文使用多个编号化 H2 主分段，"
            "并在必要时使用 H3 子分段；"
            ...
            "真实程序代码、命令输出和算法伪代码必须使用带语言标识的独立代码围栏；代码围栏内禁止使用公式定界符，"
            "也不得把代码压成一行。"
            ...
            "必要时用“注：”说明概念边界。\n"
            "11. 关系、流程、树状结构、匹配关系、位置关系或状态变化，绝不能用 ASCII、Unicode 线框字符、"
            "空格对齐或箭头在普通代码块中拼图。真正属于行列比较的数据使用 Markdown 表格；"
            "当节点与连接关系本身是理解重点时，必须输出 openclass-diagram 结构化图形围栏，不能只写“请画图”或省略图。"
            "围栏内容只能是 JSON：caption 为图题，direction 为 down/up/right/left，nodes 为 id/label 节点数组，"
            "edges 为 from/to/label 边数组。核心只按节点、边和方向渲染，不写任何主题专属规则。\n"
            "12. 抽象或严格定义第一次出现时，先给概念引入，再给正式定义；"
            "如果本节只要求建立直觉，写“注：本节只要求理解该定义所刻画的思想，暂不要求进行形式化证明。”\n"
            "13. 不写任何固定主题模板，不根据主题名、资料名或样例走特殊规则。\n"
            "14. 代码清单使用 ```<language> title=\"<清单标题>\" 开始围栏；标题描述代码在文档中的作用，"
            "编号由系统统一生成。任何代码都不得放进 LaTeX 公式。\n"
            "15. openclass-diagram 围栏格式为 {\"caption\":\"<图题>\",\"direction\":\"down\","
            "\"nodes\":[{\"id\":\"<稳定ID>\",\"label\":\"<节点文字>\"}],"
            "\"edges\":[{\"from\":\"<起点ID>\",\"to\":\"<终点ID>\",\"label\":\"<可空>\"}]}。"
            "只有信息关系需要视觉化时才使用，不把装饰性内容做成图。\n"
            "16. 练习难度若明显高于正文示例，必须给出不泄露答案的解题支架，例如终止条件、状态如何缩小、输入边界或验证步骤。"
        )
        user_payload: dict[str, Any] = {
            ...
            "response_contract": {
                ...
                "content_text": (
                    ...
                    "程序代码、命令输出和算法伪代码必须使用带语言标识的代码围栏，并可用 title 属性给出代码清单标题；"
                    "代码绝不能进入公式定界符。不要用 ASCII、Unicode 线框字符、箭头或空格对齐拼图。"
                    "当流程、树、关系或状态变化本身是理解重点时，使用 openclass-diagram JSON 围栏表达 caption、direction、nodes、edges；"
                    "最终内容不能出现“请画图”指令，也不能静默省略必要图形。"
                    "title 是唯一总标题，content_text 不得重复同名 H1；正文使用多个 H2 主分段和必要 H3 子分段，便于后续定位。"
                    "如果学习目标包含多个后续模块，正文只展开当前入口小课，其余模块放进“后续学习路线/下一步”。"
                    "章节标题优先使用 1.1/1.2 编号；正文按概念引入、正式定义、性质或结论、典型例题、解答过程、注释、习题组织。"
                    "典型例题写“例”，解答写“解：”，注释写“注：”；难度跃迁明显的练习要给通用解题支架。"
                ),
