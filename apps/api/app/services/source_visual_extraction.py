...
from app.services.source_visual_storage import SourceVisualStorageError, persist_source_visual_asset

CURRENT_SOURCE_VISUAL_INDEX_VERSION = 1
...
class SourceVisualExtractor:
    ...
    def _adapter_result(self, *, path: Path, record: SourceIngestionRecord) -> SourceVisualAdapterResult:
        ...
        if suffix in {".docx", ".pptx", ".xlsx"}:
            return extract_office_visuals(path)
    ...
    def _render_native_office_visuals(
        ...
        *,
        ...
        chart_visuals = [visual for visual in rendered.visuals if visual.kind == "diagram"]
        ...
        for anchor in anchors:
            page_matches = [
                ...
                if anchor.page_no is None or candidate.page_no == anchor.page_no
            ]
            ...
            selected.append((anchor, available.pop(matching_index), verified_mapping))
    ...
    def _materialize(
        ...
        *,
        ...
        chapters: list[SourceChapter],
        chunks: list[SourceChunk],
    ) -> tuple[list[SourceVisual], list[str]]:
...
def _chapter_for_visual(
    ...
    locator_candidates = [
        ...
        and (
            raw.source_locator.startswith(chapter.source_locator)
            or chapter.source_locator.startswith(raw.source_locator)
        )
    ]
...
def _visual_content_shape_filter_reason(
    ...
    *,
    ...
    if bbox_area is not None and bbox_area < 0.0015:
