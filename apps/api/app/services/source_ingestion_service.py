...
class SourceIngestionService:
    ...
    def queue_media_url_source(
        ...
        *,
        ...
        youtube_browser_authorization: bool = False,
    ) -> SourceIngestionRecord:
                            ...
                            "metadata": {
                                **running.metadata,
                                "media_error_code": str(
                                    getattr(exc, "code", "media_processing_failed")
                                ),
                                "media_error_retryable": bool(
        ...
        *,
        ...
        metadata = {
            **record.metadata,
            "media_operation": "full",
            "media_force_redownload": True,
            "youtube_browser_authorization": use_browser_authorization,
            "media_error_code": "",
            "media_error_retryable": False,
        }
        queued = self.store.save_source(
            record.model_copy(update={"status": "queued", "error": "", "metadata": metadata})
        )
        self._start_job(queued, adapter="media_pipeline_v1", phase="full", progress=0)
        return self._attach_media_package(self._attach_job(queued))
