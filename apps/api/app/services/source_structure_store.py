...
class SourceStructureStore:
    ...
    def visual_clues_for_scope(
        ...
        *,
        ...
        """Return incomplete visual objects as clues, never as insertable evidence."""
        ...
        return [
            asset
            for asset in (self._visual_from_row(row) for row in rows)
            if asset.anchor_status == "unverified"
            and bool(asset.metadata.get("requires_full_visual_capture"))
            and (not chapter_id or asset.chapter_id == chapter_id)
            and _visual_in_page_range(asset, page_start=page_start, page_end=page_end)
        ]

    def save_visual_codex_analysis(
        self,
        *,
        owner_user_id: str,
        package_id: str,
        ...
        """Persist analysis only when the visual identity is still unchanged."""
        with self._lock:
            with self._connect() as conn:
                row = conn.execute(
                    """
                    ...
                    WHERE owner_user_id = ? AND package_id = ?
                      ...
                      AND content_hash = ? AND position_hash = ?
                ...
                metadata["codex_visual_analysis"] = analysis
                with conn:
                    conn.execute(
                        """
                        UPDATE source_visual_assets
                        SET metadata_json = ?
                        WHERE owner_user_id = ? AND package_id = ?
                          AND source_ingestion_id = ? AND id = ?
                          AND content_hash = ? AND position_hash = ?
                ...
                updated = conn.execute(
                    ...
                    LIMIT 1
                    """,
                    (owner_user_id, package_id, source_ingestion_id, visual_id),
                ).fetchone()
        return self._visual_from_row(updated) if updated is not None else None

    def upsert_resolved_visual_asset(self, visual: SourceVisualAsset) -> SourceVisualAsset:
        """Persist one verified full-region asset produced from indexed visual clues."""

        if visual.anchor_status != "verified":
            raise ValueError("Resolved source visual must be verified before persistence.")
        if not visual.metadata.get("visual_completeness_verified"):
            raise ValueError("Resolved source visual must pass the completeness boundary.")
        old_storage_key = ""
        ...
        with self._lock:
            with self._connect() as conn:
                old_row = conn.execute(
                    """
                    SELECT storage_key FROM source_visual_assets
                    WHERE owner_user_id = ? AND package_id = ?
                      AND source_ingestion_id = ? AND id = ?
                    """,
                    (
                ...
                old_storage_key = str(old_row["storage_key"] or "") if old_row else ""
                component_ids = sorted(
                    str(value)
                    for value in visual.metadata.get("component_visual_ids", [])
                    if str(value)
                )
                superseded_rows = conn.execute(
                    """
                    SELECT id, storage_key, metadata_json FROM source_visual_assets
                    WHERE owner_user_id = ? AND package_id = ?
                      AND source_ingestion_id = ? AND page_start = ? AND id != ?
                    """,
                    (
                        visual.owner_user_id,
                        visual.package_id,
                ...
                superseded_ids = [
                    ...
                    and sorted(
                        str(value)
                        for value in _loads(row["metadata_json"], {}).get(
                            "component_visual_ids",
                            [],
                        )
                        if str(value)
                    )
                ...
                superseded_storage_keys = [
                    ...
                    for row in superseded_rows
                    if str(row["id"]) in superseded_ids and row["storage_key"]
                ]
                with conn:
                    if superseded_ids:
                        ...
                        conn.execute(
                            ...
                            WHERE owner_user_id = ? AND package_id = ?
                              AND source_ingestion_id = ? AND id IN ({placeholders})
                            """,
                            [
                                visual.owner_user_id,
                                visual.package_id,
        ...
        if old_storage_key and old_storage_key != visual.storage_key:
            self.cleanup_unreferenced_visual_assets([old_storage_key])
        if superseded_storage_keys:
            self.cleanup_unreferenced_visual_assets(superseded_storage_keys)
        return visual
