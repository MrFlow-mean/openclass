    ...
    for visual in ordered:
        ...
        items.append(
            PlannedBoardVisual(
                ...
                source=_string_value(visual, "source_title", "source_name", "source"),
                ...
                page_no=_optional_int_value(visual, "page_no"),
...
def _resolved_visual_matches_plan(item: PlannedBoardVisual, visual: Any) -> bool:
    ...
    return (
        (not resolved_id or resolved_id == item.visual_id)
        and (not item.source_ingestion_id or not source_id or source_id == item.source_ingestion_id)
        and (not item.before_chunk_id or not before or before == item.before_chunk_id)
        and (not item.after_chunk_id or not after or after == item.after_chunk_id)
    )
...
def _filter_candidates_by_source_order(
    ...
    """Keep a strictly increasing placement sequence for each source chapter.
    ...
    the candidate that breaks that invariant fails closed. Source chapters are
    isolated because a board may intentionally teach confirmed chapters in a
    different order.
    """

    by_source_chapter: dict[tuple[str, str], list[int]] = {}
    for candidate_index, (item, _placement, _marker_index) in enumerate(candidates):
        key = (item.source_ingestion_id, item.source_chapter_id)
        by_source_chapter.setdefault(key, []).append(candidate_index)

    conflicting_indexes: set[int] = set()
    for chapter_indexes in by_source_chapter.values():
        source_order = sorted(
            chapter_indexes,
            key=lambda index: (
