  ...
  return {
    ...
    studio: {
      ...
      resourcePanel: {
        ...
        queued: z ? "等待索引" : "Queued",
        processing: z ? "正在索引" : "Indexing",
        failed: z ? "索引失败" : "Index failed",
        metadataOnly: z ? "仅目录或元数据" : "Outline or metadata only",
        missingText: z ? "缺正文" : "Missing text",
        indexedBlocks: (pages: number, blocks: number) => (z ? `${pages} 页 / ${blocks} 个正文块` : `${pages} pages / ${blocks} blocks`),
