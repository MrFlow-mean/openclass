...
export function profileSettingsTexts(lang: InterfaceLanguage) {
  ...
  return {
    ...
    following: {
      ...
      lessonCountViews: (lessonCount: number, views: string) => (z ? `${lessonCount} 课 · ${views} 次浏览` : `${lessonCount} lessons · ${views} views`),
      brand: z ? "开放课堂" : "OpenClass",
      updateKinds: {
        new_lesson: z ? "新课" : "New lesson",
        course_revision: z ? "更新" : "Update",
        resource_added: z ? "资料" : "Resource",
        note_added: z ? "笔记" : "Note",
      },
      updateActions: {
        resource_added: z ? "为课程添加了资料：" : "added resources to",
        note_added: z ? "在课程中发布了笔记：" : "published notes in",
        course_revision: z ? "更新了" : "updated",
        new_lesson: z ? "发布了" : "published",
      },
      previewHeadings: {
        resource_added: z ? "资料笔记" : "Resource Notes",
        note_added: z ? "课堂笔记" : "Class Notes",
        course_revision: z ? "更新内容" : "What's Changed",
        new_lesson: z ? "要点" : "Highlights",
      },
      readMore: z ? "阅读更多" : "Read more",
    },
    ...
    trending: {
      ...
      explore: z ? "探索" : "Explore",
      topMomentum: z ? "热度最高" : "Top momentum",
      loadingOpenCourses: z ? "正在加载开放课程…" : "Loading open courses...",
      loadOpenCoursesError: z ? "无法加载开放课程" : "Could not load open courses",
      metrics: {
        stars: z ? "Stars" : "Stars",
        watchers: z ? "Watchers" : "Watchers",
        topics: z ? "Topics" : "Topics",
      },
      star: z ? "收藏" : "Star",
      starred: z ? "已收藏" : "Starred",
      lessonsCount: (count: number) => (z ? `${count} 节课` : `${count} lessons`),
      rankHash: (rank: number) => `#${rank}`,
    },
    ...
    courseDetail: {
      ...
      loading: z ? "正在加载课程…" : "Loading course...",
      loadError: z ? "无法加载开放课程" : "Could not load open course",
      fork: z ? "Fork" : "Fork",
      openForkInStudio: z ? "在工作台打开 Fork" : "Open fork in Studio",
      courseAnalysis: z ? "课程分析" : "Course analysis",
      stats: {
        forks: z ? "Forks" : "Forks",
        openImprovements: z ? "待审改进" : "Open improvements",
        lessons: z ? "Lessons" : "Lessons",
        maintainers: z ? "Maintainers" : "Maintainers",
      },
      tabs: {
        overview: z ? "概览" : "Overview",
        lessons: z ? "课程" : "Lessons",
        contributions: z ? "改进" : "Contributions",
        history: z ? "历史" : "History",
        maintainers: z ? "维护者" : "Maintainers",
      },
      maintenanceModel: z ? "维护模式" : "Maintenance model",
      maintenanceBody: z
        ? "Fork 需经课程维护者审核后，才会并入主线课程。"
        : "Forks are reviewed by course maintainers before they become part of the maintained course.",
      lessonLabel: (index: number) => (z ? `第 ${index} 课` : `Lesson ${index}`),
      commitsCount: (count: number) => (z ? `${count} 个 commit` : `${count} commits`),
      improvementTitle: z ? "改进标题" : "Improvement title",
      reviewNote: z ? "审核说明" : "Review note",
      submitImprovement: z ? "提交改进" : "Submit improvement",
      forkSubmitHint: z
        ? "此 Fork 包含课程资料的完整副本。仅在准备好提交当前 Fork 快照供维护者审核时再提交。"
      ...
      noImprovements: z ? "还没有提交任何改进。" : "No improvements have been submitted yet.",
      requestChanges: z ? "请求修改" : "Request changes",
      close: z ? "关闭" : "Close",
      merge: z ? "合并" : "Merge",
      lessonChanges: (count: number) => (z ? `${count} 处课程改动` : `${count} lesson changes`),
      resourceChanges: (count: number) => (z ? `${count} 处资料改动` : `${count} resource changes`),
      byContributor: (name: string) => (z ? `${name} 提交` : `by ${name}`),
      mainChangedAfterFork: z ? "Fork 基线之后主线已变更" : "Main changed after fork baseline",
      mergedContribution: (id: string) => (z ? `已合并改进 ${id}` : `Merged contribution ${id}`),
      addMaintainer: z ? "通过邮箱添加维护者" : "Add maintainer by email",
      add: z ? "添加" : "Add",
      addedAt: (date: string) => (z ? `添加于 ${date}` : `Added ${date}`),
      forkCreated: z ? "Fork 已创建。可在工作台继续编辑，然后从此页提交改进。" : "Fork created. Open Studio to refine it, then submit an improvement from this page.",
      improvementSubmitted: z ? "改进已提交给维护者。" : "Improvement submitted to the maintainers.",
      maintainerAdded: z ? "维护者已添加。" : "Maintainer added.",
      improvementMerged: z ? "改进已合并到课程。" : "Improvement merged into the course.",
      reviewUpdated: z ? "审核状态已更新。" : "Review updated.",
    },
