...
export const ResourceVisualBlock = Node.create({
  ...
  addAttributes() {
    return {
      ...
      pageNo: {
        default: null,
        parseHTML: (element) => visualPageNo(element.getAttribute("data-page-no")),
        renderHTML: (attributes) => {
          const pageNo = visualPageNo(attributes.pageNo);
...
function visualPageNo(value: unknown): number | null {
...
function visualSourceLabel(attrs: Record<string, unknown>): string {
  const title = visualAttr(attrs.sourceTitle) || visualAttr(attrs.source) || visualAttr(attrs.sourceLocator);
  const pageRange = visualAttr(attrs.pageRange);
  const pageNo = visualPageNo(attrs.pageNo);
  const location = pageRange || (pageNo == null ? "" : `第 ${pageNo} 页`);
