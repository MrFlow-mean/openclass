...
export function SourceImportPanel({
  ...
  const [youtubeBrowserAuthorization, setYoutubeBrowserAuthorization] = useState(false);
  const [browserAuthorizationAvailable, setBrowserAuthorizationAvailable] = useState(false);
  ...
  useEffect(() => {
    let active = true;
    void api.getMediaRuntime().then((runtime) => {
      if (active) {
        setBrowserAuthorizationAvailable(runtime.youtube_browser_authorization_enabled);
      }
    }).catch(() => {
      if (active) {
        setBrowserAuthorizationAvailable(false);
      }
    });
    return () => {
      active = false;
  ...
  async function submitUrl() {
    ...
    try {
      const record = await api.importPackageSource(packageId, {
        ...
        youtubeBrowserAuthorization: videoUrlMode && youtubeBrowserAuthorization,
      });
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-gray-200 bg-white p-3">
        ...
        <MediaUrlOptions
          ...
          browserAuthorization={youtubeBrowserAuthorization}
          browserAuthorizationAvailable={browserAuthorizationAvailable}
          onCheckedChange={setVideoUrlMode}
          onTranscriptionChange={setTranscriptionModel}
          onVisionChange={setVisionModel}
          onBrowserAuthorizationChange={setYoutubeBrowserAuthorization}
        />
...
function SourceRow({
  ...
  async function retryMedia(operation: "retranscribe" | "visuals" | "browser_authorization") {
    if (isRetrying) {
      return;
    }
    setIsRetrying(true);
    try {
      onCatalogInvalidate();
      if (operation === "browser_authorization") {
        onSourceUpdate(await api.retryMediaAccess(packageId, source.id, true));
      } else {
        onSourceUpdate(
