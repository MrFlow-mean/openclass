...
const tabIds: CourseDetailTab[] = ["overview", "lessons", "contributions", "history", "maintainers"];

function formatDate(value: string, intlLocale: string) {
...
export function OpenCourseDetailPage({ courseId }: OpenCourseDetailPageProps) {
  ...
  async function handleFork() {
    ...
    try {
      ...
      setMessage(d.forkCreated);
      await refreshDetail();
    } catch (forkError) {
      setError(forkError instanceof Error ? forkError.message : d.loadError);
    } finally {
      setBusyAction(null);
    }
  }
  ...
  async function handleSubmitContribution() {
    ...
    try {
      ...
      setMessage(d.improvementSubmitted);
      await refreshDetail();
      setActiveTab("contributions");
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : d.loadError);
    } finally {
      setBusyAction(null);
    }
  }
  ...
  async function handleReview(contributionId: string, action: "request_changes" | "close" | "merge") {
    ...
    try {
      ...
      setMessage(action === "merge" ? d.improvementMerged : d.reviewUpdated);
      await refreshDetail();
    } catch (reviewError) {
      setError(reviewError instanceof Error ? reviewError.message : d.loadError);
    } finally {
      setBusyAction(null);
    }
  }
  ...
  async function handleAddMaintainer() {
    ...
    try {
      ...
      setMaintainerEmail("");
      setMessage(d.maintainerAdded);
    } catch (maintainerError) {
      setError(maintainerError instanceof Error ? maintainerError.message : d.loadError);
    } finally {
      setBusyAction(null);
    }
  }
  ...
  const stats = [
    { label: d.stats.forks, value: formatCompactNumber(detail.course.stats.forks), Icon: GitFork },
    { label: d.stats.openImprovements, value: detail.course.stats.open_contributions.toString(), Icon: GitPullRequest },
    { label: d.stats.lessons, value: detail.course.stats.lessons.toString(), Icon: BookOpenCheck },
    { label: d.stats.maintainers, value: detail.course.stats.maintainers.toString(), Icon: UsersRound },
  ];
        ...
        <section className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_20rem]">
          ...
          <aside className="h-fit rounded-lg border border-stone-200 bg-white p-5 shadow-[0_16px_34px_rgba(15,23,42,0.05)]">
            ...
            <div className="mt-5 flex items-center gap-2 rounded-md border border-stone-200 bg-white px-3 py-3 text-sm text-stone-600">
              ...
              {d.updatedAt(formatDate(detail.course.updated_at, intlLocale))}
            </div>
          </aside>
        </section>
          ...
          <section className="rounded-lg border border-sky-200 bg-sky-50 p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
              <div className="min-w-0 flex-1">
                <label className="text-xs font-semibold uppercase text-sky-700">{d.improvementTitle}</label>
                <input
              ...
              <div className="min-w-0 flex-[1.4]">
                <label className="text-xs font-semibold uppercase text-sky-700">{d.reviewNote}</label>
                <input
            ...
            <p className="mt-3 text-xs leading-5 text-sky-800">{d.forkSubmitHint}</p>
          </section>
        ) : null}
        ...
        {activeTab === "overview" ? (
          <section className="grid gap-4 md:grid-cols-2">
            <div className="rounded-lg border border-stone-200 bg-white p-5">
              <h2 className="text-base font-semibold text-stone-950">{d.topics}</h2>
            ...
            <div className="rounded-lg border border-stone-200 bg-white p-5">
              <h2 className="text-base font-semibold text-stone-950">{d.maintenanceModel}</h2>
              <p className="mt-3 text-sm leading-6 text-stone-600">{d.maintenanceBody}</p>
            </div>
          </section>
        ) : null}
            ...
            {detail.package.lessons.map((lesson, index) => (
              <article key={lesson.id} className="rounded-lg border border-stone-200 bg-white p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold text-stone-400">{d.lessonLabel(index + 1)}</p>
                  ...
                  <span className="rounded-full bg-stone-100 px-2.5 py-1 text-xs font-semibold text-stone-600">
                    {d.commitsCount(lesson.history_graph.commits.length)}
                  </span>
                </div>
              </article>
            ))}
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2 text-xs text-stone-500">
                        <span>{d.lessonChanges(contribution.lesson_changes.length)}</span>
                        <span>{d.resourceChanges(contribution.resource_changes.length)}</span>
                        <span>{d.byContributor(contribution.contributor.display_name)}</span>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 grid gap-2 md:grid-cols-2">
                    {contribution.lesson_changes.map((change) => (
                      <div key={`${contribution.id}:${change.status}:${change.source_lesson_id}:${change.fork_lesson_id}`} className="rounded-md border border-stone-200 bg-stone-50 p-3">
                        ...
                        {change.current_changed ? <p className="mt-2 text-xs font-semibold text-amber-700">{d.mainChangedAfterFork}</p> : null}
                      </div>
                    ))}
                  </div>
                </article>
              ...
              lesson.history_graph.commits.map((commit) => (
                <article key={`${lesson.id}:${commit.id}`} className="rounded-lg border border-stone-200 bg-white p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <span className="text-xs text-stone-400">{formatDate(commit.created_at, intlLocale)}</span>
                  </div>
                  {commit.metadata?.contribution_id ? (
                    <p className="mt-3 inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700">
                      {d.mergedContribution(String(commit.metadata.contribution_id))}
                    </p>
                  ) : null}
                </article>
              ))
          ...
          <section className="space-y-3">
            ...
            {detail.maintainers.map((maintainer) => (
              <article key={maintainer.user.id} className="flex items-center justify-between rounded-lg border border-stone-200 bg-white p-4">
                ...
                <span className="text-xs text-stone-400">{d.addedAt(formatDate(maintainer.added_at, intlLocale))}</span>
              </article>
            ))}
          </section>
        ) : null}
