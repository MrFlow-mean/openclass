...
test("allows another lesson to send while a lesson chat is still streaming", async ({ page }) => {
  ...
  // This test can reuse the currently served Studio while keeping all writes in the isolated E2E API.
  await page.route("http://127.0.0.1:8000/api/**", async (route) => {
    const request = route.request();
    if (new URL(request.url()).pathname === "/api/ai-models") {
      await route.fallback();
  ...
  await expect(firstComposer).toHaveValue(`第一页先发送 ${unique}`);
  ...
  await expect(secondComposer).toHaveValue(`第二页并发发送 ${unique}`);
