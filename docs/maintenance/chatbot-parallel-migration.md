...
| P9: explicit API start generation handler prep | `codex/prep/generation-api-start-wave9-738b` | `738b378aff760d27e38a6130254f2a6a0e73b99b` | explicit `board_generation_action=start` trigger, freeze checkpoint, retryability, metadata, trace gap, standalone handler proposal, focused tests, exact call-site replacement | prep-only; no PR/merge | after Wave 8 main; keep separate from confirmed-resource, generation-control, and knowledge-board lanes | prep-only; not merged |
